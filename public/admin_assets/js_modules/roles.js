...
$.addUpdateEntity = function()
{
  ...
  var id = $.trim($('#id').val());
  var value = $("input[name=role_status]:checked").val();
  var check = true;
  var method = 'POST';
  check = validateText('#role', role, check);
...
{
  $('#role, #id').val('');
  $("input[name=role_status]:first").prop('checked','checked');
  $("input:radio").uniform();
}
